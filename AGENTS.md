...
- Set studio `casparcg.hypercomposed.pgmCameraProducer` (e.g. `dshow://video=OBS Virtual Camera`). Hypercomposed baseline keeps CAM1 warm on DoubleBox (`casparcg_pgm_camera`, ch3/115) with DoubleBox FILL so the first ILU Take is not a cold dshow open. Camera pieces still PLAY on the active look: DoubleBox FILL on ch3; fullscreen FILL on Full ch4 (`casparcg_pgm_camera_b`). ILU (`casparcg_pgm_ilu_player`, layer 116) sits above CAM so left overhang is covered without CAM cover-crop.
