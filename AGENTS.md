...
- Piece type `wipe` (megarepo `assets/`): **All** hypercomposed wiped Takes PLAY wipe on `casparcg_effects_player_pgm` (ch2/200) and hard-cut MEDIA `route://{bgA|bgB}` at `WIPE_CUT_POINT_MS` (~760 ms) — DoubleBox → ch3, Full-section (SJV / ŠPORT / Počasie / tip) → ch4. Do **not** use Caspar STING on the route (casparcg-state coerces ROUTE → `route://N-0` black PGM; STING delay units are easy to get wrong → `TRIGGER_POINT=0`). Default wipe file `wipes/wipe`; themed `wipes/wipe_sjv` / `_sport` / `_pocasie` pass through (must exist on disk).
...
- Set studio `casparcg.hypercomposed.pgmCameraProducer` (e.g. `dshow://video=OBS Virtual Camera`, or `DECKLINK DEVICE 1 FORMAT 1080p5000`). Live CAM is **not** baseline-warmed on DoubleBox: DeckLink/dshow can only open once, and Full/headline parts need the same device on ch4/115. Camera pieces PLAY on the active look only (DoubleBox FILL on ch3; fullscreen FILL on Full ch4 / `casparcg_pgm_camera_b`); the idle look's camera layer is `EMPTY` so wipe keepalive cannot hold a second open. ILU (`casparcg_pgm_ilu_player`, layer 116) sits above CAM so left overhang is covered without CAM cover-crop. DeckLink must be TSR INPUT (not quoted MEDIA). AMCP must include `DEVICE` — sofie-core Yarn-patches `casparcg-connection` so PlayDecklink emits `DECKLINK DEVICE <n>`; rebuild/restart playout-gateway to pick that up (blueprint re-upload alone is not enough).
