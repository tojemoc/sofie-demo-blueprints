...
- L3D HTML (`useStopCommand: true`, lookahead **NONE**): on Take, EMPTY the look L3D layer immediately (auto-hide previous / kill keepalive stack), then ADD the new L3D after `L3D_OUT_MS` (hard cut) or `WIPE_CUT_POINT_MS` (wiped). Same-template Takes (SJV/ŠPORT) must CLEAR then ADD — never CG UPDATE (that only swaps text). Wipe overlay starts at **0** with `previousPartKeepaliveDuration = wipeDuration` for look VIDEO only — L3Ds are CLEARed separately. Retired RE type `l3d-predstavovak` coerces to `l3d-syn` (opening → `l3d-mod`).
