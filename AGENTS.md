...
- Story looks pre-build on **BG A** (`casparcg.hypercomposed.bgChannelA`, default 3) and **BG B** (default 4). Look slots flip **only on wiped Takes** (pre-build on the idle channel); hard cuts (headlines, ILU↔SYN) stay on the current look so PGM keeps a stable `route://N` and the live camera is not thrashed across channels. Remote / Titles / Intro / DVE do not claim a look. Caspar `caspar.config` needs ≥4 channels; BG A/B are render-only (no Screen/NDI/SDI consumers — NDI on 3/4 is fine for monitoring only).
