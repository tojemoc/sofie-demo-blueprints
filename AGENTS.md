...
- Piece type `wipe` (megarepo `assets/`): **All** hypercomposed wiped Takes PLAY wipe on `casparcg_effects_player_pgm` (ch2/200) and hard-cut MEDIA `route://{bgA|bgB}` at `WIPE_CUT_POINT_MS` (~760 ms) — DoubleBox → ch3, Full-section (SJV / ŠPORT / Počasie / tip) → ch4. Wipe pieces use ≥`DEFAULT_WIPE_PREROLL_MS` (3000 ms) so Caspar can LOADBG the alpha before Take (otherwise the overlay cues ~3 s late and the cut is visible). Mixer sets `opacity: 1` (remaster `wipes/wipe.mov` if soft edges remain wrong — straight vs premultiplied). Do **not** use Caspar STING on the route (casparcg-state coerces ROUTE → `route://N-0` black PGM; STING delay units are easy to get wrong → `TRIGGER_POINT=0`). Default wipe file `wipes/wipe`; themed `wipes/wipe_sjv` / `_sport` / `_pocasie` pass through (must exist on disk).
