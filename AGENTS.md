...
- L3D HTML (`useStopCommand: true`, lookahead **NONE**): on Take, STOP the outgoing L3D (out-animation). Wiped Takes ADD the new L3D at `WIPE_CUT_POINT_MS` (under/after cover). Hard cuts wait `L3D_OUT_MS` (~200 ms) before ADD. Never CG UPDATE the same CEF instance (that only swaps text). Wipe overlay starts at **0** with `previousPartKeepaliveDuration = wipeDuration` — do not delay the sting (that tore / hard-cut before the wipe).
