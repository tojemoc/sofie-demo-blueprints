...
- Piece type `ilu-zaver`: LED 115 **windowed** (~`PGM_DOUBLEBOX_ILU_FILL`, ≈60–68%) over LED `bg_loop`. ZAVER+AVIZO is **Full look B** (PGM `route://4`, fullscreen CAM `route://5` on ch4, companion `bg_loop`, **no** `db_loop`) — never EMPTY look cam when the part owns it. Wiped ZAVER also EMPTYs Full-look ILU so `bg_pocasie` cannot linger on ch4. Do **not** `route://4` onto LED.
