 ...
 * Prefer `SOFIE_MEGAREPO_ASSETS`, then the nested megarepo path from this helpers folder.
 */
export function resolveMegarepoAsset(filename: string): string {
	...
	const legacyInRepo = resolve(here, '../../../../../assets', filename)
	throw new Error(
		`Megarepo asset not found (${filename}). Canonical copy lives in the sofie megarepo at assets/. ` +
			...
			`\n  - ${legacyInRepo} (legacy in-repo path; no longer used)`
	)
}
