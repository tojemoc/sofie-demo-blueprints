 ...
 * which pins a single sofie SHA and fails closed), then the nested megarepo path.
...
export function resolveMegarepoAsset(filename: string): string {
	...
	const candidates = [
		// Explicit override (CI / standalone checkout) — must be the pinned tree
	...
	throw new Error(
		`Megarepo asset not found (${filename}). Canonical copy lives in the sofie megarepo at assets/. ` +
			`Run scripts/fetch-sofie-megarepo-assets.sh (single pinned SHA; fails closed) or nest under sofie/. Tried:\n` +
