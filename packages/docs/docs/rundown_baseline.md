...
Hypercomposed story looks are **not** in the rundown baseline. PGM starts without a route; the first look-bearing part attaches `route://4` (Full) or `route://3` (DoubleBox) on take. Intro keeps Full underlay beneath the overlay. Logo-bug (`assets/countup`) is revealed on the first DoubleBox, still on PGM above the route.
